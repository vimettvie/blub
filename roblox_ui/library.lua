...
function Kavo.CreateLib(...)
    ...
    -- ===== [BG + SIZE + RESIZE] =====
    local MIN_W, MIN_H, MAX_W, MAX_H = 380, 240, 3000, 3000

    -- 1) Перевід фіксованого layout у відносний (щоб вікно можна було міняти в розмірі)
    MainHeader.Size = UDim2.new(1, 0, 0, 29)
    coverup.Position = UDim2.new(0, 0, 1, -7)
    coverup.Size = UDim2.new(1, 0, 0, 7)
    close.Position = UDim2.new(1, -26, 0, 4)
    MainSide.Position = UDim2.new(0, 0, 0, 29)
    MainSide.Size = UDim2.new(0, 149, 1, -29)
    coverup_2.Position = UDim2.new(1, -7, 0, 0)
    coverup_2.Size = UDim2.new(0, 7, 1, 0)
    tabFrames.Position = UDim2.new(0, 6, 0, 0)
    tabFrames.Size = UDim2.new(0, 135, 1, -6)
    pages.Position = UDim2.new(0, 157, 0, 39)
    pages.Size = UDim2.new(1, -165, 1, -49)
    blurFrame.Position = UDim2.new(0, -8, 0, -10)
    blurFrame.Size = UDim2.new(1, 16, 1, 20)
    infoContainer.Position = UDim2.new(0, 157, 1, -40)
    infoContainer.Size = UDim2.new(1, -157, 0, 33)

    -- 2) Початковий розмір: "size=400x400"
    if flags.size then
        local sw, sh = flags.size:lower():match("^(%d+)%s*[x*,]%s*(%d+)$")
        if sw then
            sw = math.clamp(tonumber(sw), MIN_W, MAX_W)
            sh = math.clamp(tonumber(sh), MIN_H, MAX_H)
            Main.Size = UDim2.new(0, sw, 0, sh)
            Main.Position = UDim2.new(0.5, -math.floor(sw / 2), 0.5, -math.floor(sh / 2))
        else
            warn("[Kavo] Невірний формат size, треба size=WxH (наприклад size=400x400)")
        end
    end

    -- 3) Якщо вікно широке — центруємо елементи на сторінці
    local function updateAlign()
        local center = pages.AbsoluteSize.X > 380
        for _, pg in ipairs(Pages:GetChildren()) do
            local l = pg:FindFirstChild("pageListing")
            if l then
                l.HorizontalAlignment = center and Enum.HorizontalAlignment.Center or Enum.HorizontalAlignment.Left
            end
        end
    end
    pages:GetPropertyChangedSignal("AbsoluteSize"):Connect(updateAlign)

    -- 4) Режим фону: панелі напівпрозорі, сторінки прозорі (елементи лишаються непрозорими)
    local bgActive = false
    local function onBackgroundReady()
        bgActive = true
        MainHeader.BackgroundTransparency = 0.4
        MainSide.BackgroundTransparency = 0.4
        coverup.BackgroundTransparency = 1
    ...
    Pages.ChildAdded:Connect(function(child)
        task.defer(function()
            updateAlign()
            if bgActive and child:IsA("ScrollingFrame") then
                child.BackgroundTransparency = 1
            end
        end)
    end)
    AttachBackground(Main, ScreenGui, themeList, flags, onBackgroundReady)

    -- 5) Куточок для зміни розміру (справа знизу)
    ...
    for _, p in ipairs({{12, 4}, {8, 8}, {12, 8}, {4, 12}, {8, 12}, {12, 12}}) do
        ...
        d.Parent = grip
        d.BackgroundColor3 = themeList.TextColor
        d.BackgroundTransparency = 0.45
        d.BorderSizePixel = 0
        d.Position = UDim2.new(0, p[1], 0, p[2])
    ...
    Main:SetAttribute("SavedSize", savedSize)
    Main:SetAttribute("SavedPosition", savedPos)
        ...
        coroutine.wrap(function()
            ...
            function Elements:NewColorPicker(colText, colInf, defcolor, callback)
                ...
                local function setcolor(tbl)
                    ...
                    cursor.Position = UDim2.new(color[1],-cx,color[2]-0.11,-cy)
                    cursor2.Position = UDim2.new(0.5,0,color[3]-0.11,-cy)
                    local realcolor = Color3.fromHSV(color[1],color[2],color[3])
                    colorCurrent.BackgroundColor3 = realcolor
                end
